...
public class PinyinUtil {
	...
	public static String convertToPinyinString(String str, String separator, PinyinFormat pinyinFormat) {
		str = ChineseUtil.convertToSimplifiedChinese(str);
		StringBuilder sb = new StringBuilder();
		for (int i = 0, len = str.length(); i < len; i++) {
			char c = str.charAt(i);
			if (ChineseUtil.isChinese(c) || c == '〇') // 判断是否为汉字或者〇
			{
				// 多音字识别处理
				...
				sb.append(c);
				if ((i + 1) < len && ChineseUtil.isChinese(str.charAt(i + 1))) {
					sb.append(separator);
				}
			}
	...
	public static String getShortPinyin(String str) {
		...
		for (int i = 0, len = str.length(); i < len; i++) {
			...
			if (!ChineseUtil.isChinese(c) && c != '〇') {
				charArray[i] = c;
			} else {
				int j = i + 1;
				sb.append(c);
				...
				// 搜索连续的汉字字符串
				while (j < len && (ChineseUtil.isChinese(str.charAt(j)) || str.charAt(j) == '〇')) {
